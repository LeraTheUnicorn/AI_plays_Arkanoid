...
from ai.ai_player_position_optimization_part1 import (
...
class TestAIPlayerPositionOptimizationPart1Mixin:
    ...
    def _create_mock_player(self):
        ...
        player.zone_handler.calculate_zones = Mock(
            return_value={"separation_zone_start": 250.0, "paddle_zone_start": 540.0}
        )
        ...
        player.trajectory_predictor.predict_paddle_intersection = Mock(
            return_value=Point(400, 550)
        )
    ...
    def test_get_optimal_paddle_position_in_separation_zone(self):
        ...
        player.trajectory_predictor.predict_paddle_intersection.return_value = Point(
            450, 550
        )
    ...
    def test_force_target_brick_from_coordinates_with_bricks(self):
        ...
        player.targeting_system = Mock()
        player.targeting_system.brick_coordinates = [
            {"x": 330, "y": 200, "brick": mock_brick}
        ]
        player.trajectory_predictor.predict_paddle_intersection.return_value = Point(
            400, 550
        )
        player.trajectory_predictor.predict_after_bounce_trajectory = Mock(
            return_value=[Point(300, 200)]
        )
        player.config = Mock()
