...
class AIPlayerMovementHelpersMixin:
    ...
    def _execute_movement_strategy(self, current_x: int, paddle_speed: int) -> int:
        ...
        if self.paddle_movement_strategy is None:
            self._logger.warning(
                "[AI_PLAYER_MOVE] paddle_movement_strategy не инициализирован, используем fallback"
            )
            return self._fallback_movement(current_x)
    ...
    def _calculate_optimal_position(self) -> int:
        ...
        current_x = (
            int(self.current_game_state.paddle_position.x)
            if self.current_game_state
            else 0
        )
        self._logger.debug(
            f"[PADDLE DEBUG] ball_y={ball_y:.1f}, ball_vel_y={ball_vel_y}, current_x={current_x}, optimal_x={optimal_x}, distance={abs(current_x - optimal_x):.1f}"
        )
