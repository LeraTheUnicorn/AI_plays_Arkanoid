    ...
    def _apply_movement_strategy_part1(
        ...
        if ball_vel_y == 0:
            ...
            self._log_paddle_movement(
                current_x, current_x, "ball_vel_y_zero_warning", 0.5
            )
        ...
        if ball_y < separation_zone_start and not is_last_brick:
            ...
            self._logger.debug(
                f"[PADDLE DEBUG] ПРАВИЛО 2: Мяч в зоне кубиков (ball_y={ball_y:.1f} < {separation_zone_start}), платформа не двигается"
            )
            ...
            # Фильтрация по уровню выполняется автоматически системой логирования Python
            self._logger.debug(
                f"[PADDLE DEBUG] ПРАВИЛО 2 EXCEPTION: Последний кирпич! Разрешаем движение в зоне кубиков"
            )
        ...
        if ball_lost:
            ...
            # Фильтрация по уровню выполняется автоматически системой логирования Python
            self._logger.debug(
                f"[PADDLE DEBUG] Мяч потерян (ball_y={ball_y:.1f} > paddle_y={paddle_y:.1f}), платформа не двигается"
            )
            self._log_paddle_movement(
        ...
        if should_log:  # Всегда логируем когда мяч близко
            ...
            ball_speed = (
                self.current_game_state.ball_speed if self.current_game_state else 0
            )
            distance_to_paddle = paddle_y - ball_y if ball_y < paddle_y else 0
            time_to_paddle = (
                distance_to_paddle / ball_vel_y
                if ball_vel_y > 0 and distance_to_paddle > 0
                else float("inf")
            )
                ...
                if target_pos is not None:
                    ...
                    if self.performance_monitor and start_time_monitor:
                        ...
                        self.performance_monitor.record_metric(
                            "get_optimal_paddle_position", duration
                        )
            else:
                optimal_x = self.get_optimal_paddle_position()
            distance_to_target = (
                abs(current_x - optimal_x) if optimal_x is not None else 0
            )
            self._logger.debug(
                f"[BALL TRACKING] ball=({ball_x:.1f},{ball_y:.1f}) vel=({ball_vel_x:.1f},{ball_vel_y:.1f}) speed={ball_speed:.1f} | "
        ...
        ball_x = (
            self.current_game_state.ball_position.x if self.current_game_state else 0
        )
        current_vel_x = (
            self.current_game_state.ball_velocity.x
            if (
        ...
        if (
            ...
            vel_y_direction_change = (
                last_vel_y > 0 and current_vel_y < 0
            )  # Мяч двигался вниз, теперь вверх
            ...
            if (
                position_change > 50
                or (y_change < -10 and ball_y < separation_zone_start)
                or vel_y_direction_change
            ):
                ...
                if (
                    vel_x_change < 5 and position_change > 50
                ) or vel_y_direction_change:
                    brick_bounce_detected = True
                    bounce_reason = (
                        "vel_y direction change"
                        if vel_y_direction_change
                        else f"position change {position_change:.1f}px"
                    )
                    self._logger.debug(
        ...
        if self.separation_zone_tracker.target_position_set:
            ...
            if current_target is not None and in_separation_zone:
                ...
                if (
                    saved_vel_x is not None and abs(current_vel_x - saved_vel_x) > 0.1
                ) or brick_bounce_detected:
                    # Мяч отскочил от стены или от кирпича - траектория изменилась, нужно пересчитать цель
                    if brick_bounce_detected:
                        self._logger.debug(
                            f"[TARGET RESET] Мяч отскочил от кирпича! Траектория изменилась, пересчитываем цель"
                        )
                    else:
                        self._logger.debug(
                            f"[TARGET RESET] Мяч отскочил от стены! Старая vel_x={saved_vel_x:.1f}, Новая vel_x={current_vel_x:.1f}"
                        )
                    ...
                    if time_to_paddle != float("inf") and time_to_paddle > 0:
                        ...
                        if distance_to_new_target > 200:
                            ...
                            max_distance = (
                                paddle_speed * time_to_paddle
                                if time_to_paddle < 100
                                else 200
                            )

                            # Выбираем промежуточную позицию в направлении новой цели
                            if zone_center_x > current_x:
                                conservative_target = min(
                                    zone_center_x, current_x + max_distance
                                )
                            else:
                                conservative_target = max(
                                    zone_center_x, current_x - max_distance
                                )
                            ...
                            conservative_target = max(
                                min_x, min(max_x, conservative_target)
                            )

                            self._logger.debug(
                                f"[CONSERVATIVE TARGET] Большое расстояние ({distance_to_new_target:.1f}px), "
                            ...
                            self.separation_zone_tracker.target_position = int(
                                conservative_target
                            )
                            self.separation_zone_tracker.target_position_set = True
                            self.separation_zone_tracker.saved_ball_vel_x = (
                                current_vel_x
                            )
                    ...
                    self._log_paddle_movement(
                        current_x, current_x, "target_reset_wall_bounce", 1.0
                    )
                    ...
                    tolerance = (
                        15  # Равен скорости движения платформы - предотвращает дергание
                    )

                    # КРИТИЧНО: Логируем движение к зафиксированной позиции для отслеживания
                    # Фильтрация по уровню выполняется автоматически системой логирования Python
                    self._logger.debug(
                        f"[MOVING TO FIXED] ФЛАГ: Движение к зафиксированной позиции! "
                    ...
                    if distance_to_target > tolerance:
                        ...
                        if movement != 0:
                            self._update_loop_tracking(
                                movement, int(current_x), int(target_pos)
                            )
            ...
            if should_reset:
                ...
                self.separation_zone_tracker.paddle_reached_target = False
                self._log_paddle_movement(
                    current_x, current_x, "target_reset_ball_left_zone", 1.0
                )
                    ...
                    if distance_to_target > tolerance:
                        ...
                        if movement != 0:
                            ...
                            self._log_paddle_movement(
                                current_x,
                                target_pos,
                                "moving_to_fixed_target_outside_zone",
                                1.0,
                            )
                            return movement
                    else:
