...
class AIPlayerDebugVisualizationMixin:
    ...
    def _draw_predicted_trajectory(self, screen: Any) -> None:
        ...
        try:
            ...
            self._logger.debug(
                f"Ошибка типов при отрисовке траектории: {e}", exc_info=True
            )
        except (ImportError, NameError) as e:
            # Игнорируем ошибки импорта pygame
            self._logger.debug(
                f"Ошибка импорта при отрисовке траектории: {e}", exc_info=True
            )
        except Exception as e:
            # Игнорируем ошибки предсказания при отрисовке
            if hasattr(self, "_logger"):
                self._logger.debug(
                    f"Ошибка предсказания при отрисовке: {e}", exc_info=True
                )
