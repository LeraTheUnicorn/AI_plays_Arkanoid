...
class GameZones:
    ...
    ball_reset_height: int = (
        50  # Высота, выше которой сбрасывается отслеживание зоны разделения
    )
...
class PaddleConfig:
    ...
    edge_proximity_threshold: int = (
        40  # Порог близости к краю экрана для прямого прицеливания
    )
...
class BrickConfig:
    ...
    precision_targeting_threshold: int = (
        10  # Порог количества кирпичей для точного прицеливания
    )
...
class BallConfig:
    ...
    velocity_tolerance: float = (
        0.1  # Допустимое отклонение скорости для определения отскока
    )
...
class AIConfig:
    ...
    recent_targets_max: int = (
        5  # Максимальное количество последних целей для отслеживания
    )
    successful_hits_max: int = 100  # Максимальное количество успешных ударов в истории
    successful_hits_keep: int = (
        50  # Количество успешных ударов для сохранения после очистки
    )
