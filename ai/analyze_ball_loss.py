...
def parse_ball_loss_log(log_text: str) -> List[Dict[str, Any]]:
    ...
    for match in matches:
        ...
        ball_match = re.search(
            r"Мяч: pos=\(([\d.]+), ([\d.]+)\) bottom=([\d.]+) vel=\(([-\d.]+), ([\d.]+)\) speed=([\d.]+)",
            match,
        )
        if ball_match:
        ...
        paddle_match = re.search(
            r"Платформа: center_x=([\d.]+) top=([\d.]+) left=([\d.]+) right=([\d.]+)",
            match,
        )
        if paddle_match:
        ...
        dist_match = re.search(
            r"Расстояние: horizontal=([\d.]+)px vertical=([\d.]+)px", match
        )
        if dist_match:
        ...
        zone_match = re.search(
            r"Мяч относительно платформы: ([^(]+) \(offset=([-\d.]+)px\)", match
        )
        if zone_match:
        ...
        optimal_match = re.search(
            r"Целевая позиция AI: optimal_x=([\d.]+) distance_to_optimal=([\d.]+)px",
            match,
        )
        if optimal_match:
        ...
        pred_match = re.search(
            r"🔴 ПРЕДСКАЗАНИЕ: Предсказанная позиция приземления: ([\d.]+)px", match
        )
        if pred_match:
            loss_data["predicted_landing_x"] = float(pred_match.group(1))

        actual_match = re.search(
            r"🔴 ПРЕДСКАЗАНИЕ: Фактическая позиция мяча: ([\d.]+)px", match
        )
        if actual_match:
            loss_data["actual_ball_x"] = float(actual_match.group(1))

        error_match = re.search(
            r"🔴 ПРЕДСКАЗАНИЕ: Ошибка предсказания: ([\d.]+)px", match
        )
        if error_match:
            loss_data["prediction_error"] = float(error_match.group(1))

        miss_match = re.search(
            r"🔴 ПРЕДСКАЗАНИЕ: Мяч пролетел мимо на: ([\d.]+)px от центра платформы",
            match,
        )
        if miss_match:
        ...
        zones_match = re.search(
            r"Зоны: separation_start=(\d+) paddle_start=(\d+) ball_was_in_zone=(True|False)",
            match,
        )
        if zones_match:
        ...
        target_set_match = re.search(
            r"Целевая позиция установлена: (True|False)", match
        )
        if target_set_match:
            loss_data["target_position_set"] = target_set_match.group(1) == "True"

        saved_target_match = re.search(
            r"Сохраненная целевая позиция: ([\d.]+) distance=([\d.]+)px", match
        )
...
def analyze_losses(losses: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    distances = [
        loss.get("horizontal_distance", 0)
        for loss in losses
        if "horizontal_distance" in loss
    ]
    if distances:
    ...
    prediction_errors = [
        loss.get("prediction_error", 0) for loss in losses if "prediction_error" in loss
    ]
    if prediction_errors:
        stats_dict = cast(Dict[str, Any], analysis["statistics"])
        stats_dict["avg_prediction_error"] = sum(prediction_errors) / len(
            prediction_errors
        )
    ...
    distances_to_optimal = [
        loss.get("distance_to_optimal", 0)
        for loss in losses
        if "distance_to_optimal" in loss
    ]
    if distances_to_optimal:
        stats_dict = cast(Dict[str, Any], analysis["statistics"])
        stats_dict["avg_distance_to_optimal"] = sum(distances_to_optimal) / len(
            distances_to_optimal
        )
    ...
    if target_differences:
        stats_dict = cast(Dict[str, Any], analysis["statistics"])
        stats_dict["avg_target_difference"] = sum(target_differences) / len(
            target_differences
        )
            ...
            if distance > 200 and speed > 0:  # Большое расстояние
                ...
                if "ball_y" in loss and "paddle_start" in loss and "ball_vel_y" in loss:
                    ...
                    if ball_vel_y > 0:
                        ...
                        frames_to_reach = (
                            distance / speed if speed > 0 else float("inf")
                        )
                        if (
                            frames_to_reach > time_to_paddle * 1.2
                        ):  # Платформа не успеет
                            issues_list = cast(List[Dict[str, Any]], analysis["issues"])
                            issues_list.append(
                                {
    ...
    for loss in losses:
        if (
            loss.get("target_position_set")
            and "saved_target_position" in loss
            and "optimal_x" in loss
        ):
...
def print_analysis(analysis: Dict[str, Any]) -> None:
    ...
    if "statistics" in analysis:
        ...
        if "avg_horizontal_distance" in stats:
            print(
                f"Среднее горизонтальное расстояние: {stats['avg_horizontal_distance']:.1f}px"
            )
