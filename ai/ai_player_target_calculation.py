...
class AIPlayerTargetCalculationMixin:
    ...
    def _calculate_target_position(
        ...
        ball_speed = (
            ...
            if self.current_game_state
            else self.config.ball.default_speed
        )
        ...
        precision_priority = (
            user_rules.get("precision_priority", False)
            or bricks_count <= self.config.precision_priority_threshold
            or bricks_count <= 1  # Принудительно для последнего кирпича
        )
        ...
        if (
            self.empty_bounce_tracker["consecutive_empty_bounces"]
            ...
            if self.targeting_system.brick_coordinates:
                ...
                if optimal_position is not None:
                    ...
                    optimal_position = self._ensure_safe_paddle_position(
                        float(optimal_position), landing_x
                    )
                    optimal_position = max(
                        min_position, min(max_position, int(optimal_position))
                    )
                    ...
                    in_separation_zone = (
                        zones["separation_zone_start"]
                        <= ball_y
                        < zones["paddle_zone_start"]
                    )
        ...
        if precision_priority:
            if not self.current_game_state:
                return self._calculate_fallback_position(
                    landing_x, ball_y, zones, bricks_count
                )
            ...
            optimal_position = (
                self.position_calculator.calculate_precise_position_for_few_bricks(
            ...
            return self._calculate_precision_position(
                landing_x, ball_y, zones, bricks_count, ball_speed, user_rules
            )
        ...
        if bricks_count <= 15:
            ...
            optimal_position = (
                self.position_calculator.calculate_position_for_max_destruction(
                    landing_x, self.current_game_state
                )
            )
        ...
        if target_brick:
            optimal_position = (
                self.position_calculator.calculate_position_with_target_brick(
    ...
    def _calculate_precision_position(
        ...
        if bricks_count == 1:
            ...
            if target_brick:
                ...
                if in_separation_zone:
                    self._set_target_position_if_needed(
                        int(optimal_position), "last_brick_precision"
                    )
        ...
        if optimal_position is None:
            if self.targeting_system.brick_coordinates:
                ...
                if self.current_game_state and self.current_game_state.remaining_bricks:
                    ...
                    required_offset = max(
                        -1.5, min(1.5, dx / (paddle_half_width * 1.5))
                    )
        ...
        if optimal_position is not None:
            ...
            if (
                success_prob > self.config.low_success_probability_threshold
                or bricks_count <= self.config.precision_priority_threshold
            ):
                in_separation_zone = (
                    zones["separation_zone_start"]
        ...
        if target_brick:
            ...
            if position_preference < 0.4:
                if not self.separation_zone_tracker.target_position_set:
                    for offset in range(
                        self.config.paddle_zone_offset_range[0],
                        self.config.paddle_zone_offset_range[1],
                        self.config.paddle_zone_offset_step,
                    ):
    ...
    def _ensure_safe_paddle_position(
        ...
        safe_edge_distance = (
            25  # Минимальное расстояние от края платформы до точки попадания мяча
        )
    ...
    def _calculate_fallback_position(
        ...
        if (
            bricks_count <= self.config.precision_priority_threshold
            and self.targeting_system.brick_coordinates
        ):
            if self.separation_zone_tracker.target_position_set:
                target_pos = self.separation_zone_tracker.target_position
                if target_pos is not None:
            ...
            if optimal_position is not None:
                ...
                if in_separation_zone:
                    self._set_target_position_if_needed(
                        int(optimal_position), "brick_coords"
                    )
        ...
        # КРИТИЧНО: Применяем защиту от попадания в углы платформы
        base_position = self._ensure_safe_paddle_position(
            float(base_position), landing_x
        )
    ...
    def _set_target_position_if_needed(self, position: int, reason: str) -> None:
        ...
        self.target_tracker.set_target_position(
            position, reason, self._logger, current_pos
        )
