...
async def example_async_trajectory_prediction():
    ...
    async_predictor = AsyncTrajectoryPredictor(
        screen_width=800, screen_height=600, max_workers=2
    )
    ...
    trajectory = await async_predictor.predict_trajectory_async(
        game_state, max_points=50
    )
    print(f"Предсказано точек траектории: {len(trajectory)}")

    # Асинхронное предсказание пересечения с платформой
    intersection = await async_predictor.predict_paddle_intersection_async(
        game_state, paddle_y=550.0
    )
    if intersection:
    ...
    tasks = [
        ...
        async_predictor.get_optimized_trajectory_async(
            game_state, max_relevant_points=30
        ),
    ]
...
def example_sync_usage():
    ...
    intersection = async_predictor.predict_paddle_intersection_sync(
        game_state, paddle_y=550.0
    )
    if intersection:
