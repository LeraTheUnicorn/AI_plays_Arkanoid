...
def analyze_recent_performance(results: List[Dict[str, Any]], window: int = 50) -> Dict[str, Any]:
    ...
    return {
        ...
        "winrate": (sum(1 for g in recent if g.get("success", False)) / len(recent) * 100) if recent else 0,
...
def print_analysis_report(results: List[Dict[str, Any]]):
    ...
    print(f"   Последние 50 игр: {winrate_trend['recent_50_winrate']:.2f}% ({winrate_trend['recent_50_games']} игр)")
    print(f"   Последние 100 игр: {winrate_trend['recent_100_winrate']:.2f}% ({winrate_trend['recent_100_games']} игр)")
    print(f"   Последние 200 игр: {winrate_trend['recent_200_winrate']:.2f}% ({winrate_trend['recent_200_games']} игр)")
    print()
    ...
    print(f"   Максимум последовательных поражений: {recent_perf['max_consecutive_losses']}")
    ...
    if loss_patterns.get("total_losses", 0) > 0:
        ...
        for bricks, count in sorted(loss_patterns['bricks_remaining_distribution'].items()):
            print(f"      {bricks} кирпичей: {count} игр ({count/loss_patterns['total_losses']*100:.1f}%)")
        print(f"   Распределение по счету:")
        for score_range, count in loss_patterns['score_distribution'].items():
            if count > 0:
                print(f"      {score_range}: {count} игр ({count/loss_patterns['total_losses']*100:.1f}%)")
        print(f"   Средняя длительность поражения: {loss_patterns['average_duration']:.2f} сек")
        print(f"   Мин/Макс длительность: {loss_patterns['min_duration']:.2f} / {loss_patterns['max_duration']:.2f} сек")
        print()
    ...
    if victory_patterns.get("total_victories", 0) > 0:
        ...
        if victory_patterns.get("anomalies_with_remaining_bricks", 0) > 0:
            print(f"   ⚠️  АНОМАЛИИ: {victory_patterns['anomalies_with_remaining_bricks']} побед с оставшимися кирпичами!")
        print(f"   Средняя длительность победы: {victory_patterns['average_duration']:.2f} сек")
        print(f"   Мин/Макс длительность: {victory_patterns['min_duration']:.2f} / {victory_patterns['max_duration']:.2f} сек")
        print()
    ...
    if recent_losses:
        ...
        for i, loss in enumerate(recent_losses[-10:], 1):
            ...
            print(f"   {i}. Счет: {score}, Осталось кирпичей: {bricks}, Длительность: {duration:.1f}с")
        print()
    ...
    if winrate_trend['recent_50_winrate'] < 100:
        ...
        if loss_patterns.get("total_losses", 0) > 0:
            ...
            if 1 in bricks_dist and bricks_dist[1] > loss_patterns['total_losses'] * 0.5:
        ...
        if recent_perf['max_consecutive_losses'] >= 3:
            print(f"   ⚠️  Обнаружена серия из {recent_perf['max_consecutive_losses']} последовательных поражений")
