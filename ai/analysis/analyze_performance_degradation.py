...
class LogAnalyzer:
            ...
            for line in f:
                ...
                if new_target_match:
                    ...
                    action_match = re.search(
                        r"Расстояние до цели слишком мало.*?(\d+\.?\d*)px", line
                    )
                    if action_match:
    ...
    def analyze_performance(self) -> Dict[str, any]:
        ...
        analysis = {
            ...
            "new_targets": len(
                [e for e in self.movement_events if e.movement_type == "NEW_TARGET"]
            ),
            "fixed_targets": len(
                [e for e in self.movement_events if e.movement_type == "FIXED_TARGET"]
            ),
        ...
        large_distances = [
            e for e in self.movement_events if e.distance and e.distance > 400
        ]
        if large_distances:
            avg_large_distance = sum(
                e.distance for e in large_distances if e.distance
            ) / len(large_distances)
            analysis["issues"].append(
                {
        ...
        for bounce in self.bounce_events:
            ...
            if not target_reset:
                bounce_reactions.append(
                    {"bounce": bounce, "reaction_delay": len(next_events)}
                )
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"  - Зафиксированных целей (FIXED_TARGET): {analysis['fixed_targets']}"
        )
        ...
        if analysis["issues"]:
            ...
            for i, issue in enumerate(analysis["issues"], 1):
                ...
                report.append(
                    f"{i}. {severity_mark} [{issue['severity']}] {issue['type']}"
                )
