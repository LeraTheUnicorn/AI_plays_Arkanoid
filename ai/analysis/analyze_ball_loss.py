...
def analyze_text_logs(log_file: str) -> Dict[str, Any]:
    ...
    for i, line in enumerate(lines):
        ...
        # Отбитие в пустоту
        if "пустот" in line.lower() or "empty" in line.lower() or "отбитие" in line.lower():
        ...
        # Точное прицеливание
        if "precision" in line.lower() or "точное" in line.lower() or "few_bricks" in line.lower():
    ...
    return {
        ...
        "recent_empty_bounce": empty_bounce_patterns[-10:] if empty_bounce_patterns else [],
        "recent_precision": precision_targeting_patterns[-10:] if precision_targeting_patterns else [],
...
def print_analysis():
    ...
    if os.path.exists(logs_dir):
        ...
        if json_files:
            latest_json = max(json_files, key=lambda f: os.path.getmtime(os.path.join(logs_dir, f)))
            ...
            if json_analysis:
                ...
                print(f"   Событий потери мяча: {json_analysis.get('ball_loss_events', 0)}")
                print(f"   Попаданий в кирпичи: {json_analysis.get('brick_hits_count', 0)}")
                print(f"   Движений платформы: {json_analysis.get('paddle_movements_count', 0)}")
                print()
    ...
    if os.path.exists(logs_dir):
        ...
        for item in os.listdir(logs_dir):
            ...
            if os.path.isdir(item_path):
                ...
                if log_files:
                    latest_log = max(log_files, key=lambda f: os.path.getmtime(os.path.join(item_path, f)))
                    ...
                    if text_analysis:
                        ...
                        print(f"   Упоминаний потери мяча: {text_analysis.get('ball_lost_count', 0)}")
                        print(f"   Упоминаний отбития в пустоту: {text_analysis.get('empty_bounce_count', 0)}")
                        print(f"   Упоминаний точного прицеливания: {text_analysis.get('precision_targeting_count', 0)}")
                        print(f"   Упоминаний последнего кирпича: {text_analysis.get('last_brick_count', 0)}")
                        print()
                        
                        if text_analysis.get('recent_empty_bounce'):
                            print("🔴 ПОСЛЕДНИЕ ОТБИТИЯ В ПУСТОТУ:")
                            for item in text_analysis['recent_empty_bounce'][-5:]:
                                print(f"   Строка {item['line']}: {item['content'][:100]}")
                            print()
