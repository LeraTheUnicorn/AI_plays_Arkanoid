...
1. **Поток данных** - как информация движется через систему с учетом всех компонентов
...
Диаграмма соответствует текущей реализации кода:

### Основные компоненты:
- ✅ **AIPlayer** - координатор всех компонентов системы
- ✅ **TrajectoryPredictor** - предсказание траекторий мяча
- ✅ **PositionOptimizer** - оптимизация позиции платформы
- ✅ **LearningSystem** - система машинного обучения с кластеризацией и классификацией
- ✅ **PerformanceLogger** - логирование всех действий и результатов
- ✅ **TargetingSystem** - система прицельного отбивания с динамической картой кубиков
- ✅ **LoopPreventionSystem** - предотвращение зацикливания с порогом 80%
- ✅ **CeilingBounceSystem** - специальная логика для отскоков от потолка

### Ключевые принципы:

- **Модульность** - каждый компонент отвечает за свою область с четким разделением ответственности
- **Обучение** - система улучшается на основе опыта с сохранением паттернов попаданий в `ai/models/ai_model.json`
...
- **Адаптивность** - специальная логика для различных сценариев (мало кубиков ≤10, отскоки от потолка)
- **Точность** - строгое ограничение границ экрана (±5px) и предотвращение вертикальных ударов
- **Динамическая адаптация** - в Training Mode скорость мяча адаптируется в диапазоне 1-15
- **Многопоточность** - асинхронное логирование и предсказание траекторий для оптимизации производительности
