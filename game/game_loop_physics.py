...
def update_ball_physics(
    ...
    if (
        ...
        and not (
            just_bounced
            and (bounce_frame == frame_counter or bounce_frame == frame_counter - 1)
        )
    ):
...
def check_paddle_collisions(
    ...
    try:
        ...
        if just_bounced and (
            bounce_frame == frame_counter or bounce_frame == frame_counter - 1
        ):
            ...
            ball_hits_paddle_top = (
                ...
                and paddle.rect.left - 5
                <= ball.rect.centerx
                <= paddle.rect.right
                + 5  # Мяч по горизонтали в пределах платформы (с запасом 5px)
                and ball.rect.bottom
                >= paddle.rect.top  # Нижняя часть мяча касается или ниже верхней части платформы
                and ball.rect.bottom
                <= paddle.rect.top + 15  # Мяч в пределах 15 пикселей от верха платформы
                and ball.rect.top
                < paddle.rect.top
                + 10  # КРИТИЧНО: Мяч не слишком глубоко внутри платформы (верхняя часть мяча не ниже 10px от верха платформы)
            )
        if frame_counter <= 3:
        ...
        ball_hits_paddle_side = (
            ...
            and (
                ...
                (
                    ...
                    and ball.rect.right <= paddle.rect.left + 10
                    and ball.rect.centerx < paddle.rect.left
                )
                or
                # Мяч касается правой стороны платформы
                (
                    ball.rect.left <= paddle.rect.right
                    and ball.rect.left >= paddle.rect.right - 10
                    and ball.rect.centerx > paddle.rect.right
                )
                or
                ...
                (
                    ...
                    and (
                        ...
                        or ball.rect.centerx > paddle.rect.right
                    )
                )
            )
        )
    ...
    if frame_counter <= 3:
        logger.debug(
            f"[AI DEBUG] Проверяем условия ball_hits_paddle_side и ball_hits_paddle_top..."
        )
        try:
            logger.debug(
                f"[AI DEBUG] ball_hits_paddle_side={ball_hits_paddle_side}, ball_hits_paddle_top={ball_hits_paddle_top}"
            )
        except Exception as e:
            logger.error(f"[ERROR] Ошибка при выводе значений: {e}")
            logger.debug(
                f"[AI DEBUG] ball_hits_paddle_side type: {type(ball_hits_paddle_side) if 'ball_hits_paddle_side' in locals() else 'NOT DEFINED'}"
            )
            logger.debug(
                f"[AI DEBUG] ball_hits_paddle_top type: {type(ball_hits_paddle_top) if 'ball_hits_paddle_top' in locals() else 'NOT DEFINED'}"
            )
    ...
    if frame_counter <= 3:
        logger.debug(
            f"[AI DEBUG] ball_hits_paddle_side=False, проверяем прилипание мяча..."
        )
    try:
        ...
        ball_stuck = (
            ball.rect.colliderect(
                paddle.rect
            )  # Мяч ВНУТРИ платформы (пересекается с ней)
...
def handle_paddle_top_bounce(
    ...
    if training_mode and ai_player is not None:
        try:
            ...
            if (
                hasattr(ai_player, "trajectory_predictor")
                and hasattr(ai_player, "current_game_state")
                and ai_player.current_game_state is not None
            ):
                try:
                    intersection_point = (
                        ai_player.trajectory_predictor.predict_paddle_intersection(
                            ai_player.current_game_state, paddle.rect.centery
                        )
                    )
                    ...
                    try:
                        predicted_landing_x = (
                            ai_player._predict_exact_landing_position()
                        )
                    except:
                        pass
...
def handle_ball_stuck(
    ...
    ball.rect.centery = (
        paddle.rect.top - ball_radius - 15
    )  # Увеличиваем расстояние для надежности
    # КРИТИЧНО: Убеждаемся, что мяч не находится внутри платформы
    if ball.rect.colliderect(paddle.rect):
        # Если мяч все еще внутри платформы, перемещаем его еще выше
...
def check_brick_collisions(
    ...
    if hit_index != -1:
        ...
        if training_mode and ai_player is not None:
            ceiling_bounces = (
                ai_player.empty_bounce_tracker.get("ceiling_bounces", 0) or 0
            )
    ...
    if ball.rect.bottom > paddle.rect.top and not ball_hits_paddle_top:
        ...
        if training_mode and ai_player is not None:
            if not getattr(sys, "frozen", False):
                ...
                optimal_x = (
                    ai_player.get_optimal_paddle_position()
                    if hasattr(ai_player, "get_optimal_paddle_position")
                    else paddle_x
                )
                distance_to_optimal = (
                    abs(paddle_x - optimal_x) if optimal_x is not None else 0
                )
                ...
                try:
                    if hasattr(ai_player, "trajectory_predictor") and hasattr(
                        ...
                        if current_state is not None:
                            try:
                                intersection_point = ai_player.trajectory_predictor.predict_paddle_intersection(
                                    current_state, paddle.rect.centery
                                )
                                if intersection_point:
                                    predicted_landing_x = intersection_point.x
                ...
                separation_zone_start = (
                    ai_player.separation_zone_tracker.separation_zone_start
                    if hasattr(ai_player, "separation_zone_tracker")
                    else 226
                )
                paddle_zone_start = (
                    ai_player.separation_zone_tracker.paddle_zone_start
                    if hasattr(ai_player, "separation_zone_tracker")
                    else 540
                )
                ...
                adjusted_speed = (
                    ai_player.get_adjusted_paddle_speed(base_speed)
                    if hasattr(ai_player, "get_adjusted_paddle_speed")
                    else base_speed
                )

                # Рассчитываем, где должна была быть платформа
                ball_was_in_separation_zone = (
                    separation_zone_start <= ball_y < paddle_zone_start
                )
                ...
                ball_hit_paddle = (
                    ...
                    and ball_bottom <= paddle_top + 5
                )

                print(f"[BALL LOST] ========== ДИАГНОСТИКА ПОТЕРИ МЯЧА ==========")
                print(
                    f"  Мяч: pos=({ball_x:.1f}, {ball_y:.1f}) bottom={ball_bottom:.1f} vel=({ball_vel_x:.1f}, {ball_vel_y:.1f}) speed={ball_speed:.1f}"
                )
                print(
                    f"  Платформа: center_x={paddle_x:.1f} top={paddle_top:.1f} left={paddle_left:.1f} right={paddle_right:.1f}"
                )
                print(
                    f"  Расстояние: horizontal={horizontal_distance:.1f}px vertical={ball_bottom - paddle_top:.1f}px"
                )
                print(
                    f"  Мяч относительно платформы: {ball_zone} (offset={ball_offset_from_paddle_center:.1f}px)"
                )
                print(
                    f"  Мяч ударился о платформу: {ball_hit_paddle} (если False - мяч пролетел мимо)"
                )
                print(
                    f"  Целевая позиция AI: optimal_x={optimal_x:.1f} distance_to_optimal={distance_to_optimal:.1f}px"
                )
                if predicted_landing_x is not None:
                    print(
                        f"  [ПРЕДСКАЗАНИЕ] Предсказанная позиция приземления: {predicted_landing_x:.1f}px"
                    )
                    print(f"  [ПРЕДСКАЗАНИЕ] Фактическая позиция мяча: {ball_x:.1f}px")
                    print(
                        f"  [ПРЕДСКАЗАНИЕ] Ошибка предсказания: {prediction_error:.1f}px"
                    )
                    print(
                        f"  [ПРЕДСКАЗАНИЕ] Мяч пролетел мимо на: {abs(ball_x - paddle_x):.1f}px от центра платформы"
                    )
                else:
                    print(f"  [ПРЕДСКАЗАНИЕ] Предсказанная позиция НЕ ДОСТУПНА")
                print(
                    f"  Скорость платформы: base={base_speed} adjusted={adjusted_speed}"
                )
                print(
                    f"  Зоны: separation_start={separation_zone_start} paddle_start={paddle_zone_start} ball_was_in_zone={ball_was_in_separation_zone}"
                )
                print(
                    f"  Целевая позиция установлена: {ai_player.separation_zone_tracker.target_position_set if hasattr(ai_player, 'separation_zone_tracker') else False}"
                )
                if (
                    hasattr(ai_player, "separation_zone_tracker")
                    and ai_player.separation_zone_tracker.target_position
                ):
                    target_pos = ai_player.separation_zone_tracker.target_position
                    if target_pos is not None:
                        print(
                            f"  Сохраненная целевая позиция: {target_pos:.1f} distance={abs(paddle_x - target_pos):.1f}px"
                        )
                print(f"========================================================")
...
def handle_game_restart_training(
    ...
    if not getattr(sys, "frozen", False):
        ...
        direction_text = (
            "влево"
            if new_ball.vel_x < 0
            else "вправо" if new_ball.vel_x > 0 else "прямо"
        )
        print(f"[GAME RESTART] Игра перезапущена после {restart_type}!")
        print(
            f"[GAME RESTART] Мяч: скорость={new_ball.get_speed()}, направление={direction_text} ({direction_info}), угол={angle_degrees}°, vel_x={new_ball.vel_x}, vel_y={new_ball.vel_y} | Платформа: x={new_paddle.rect.x} | Кирпичей: {len(new_bricks)}"
        )
...
def handle_paddle_side_collision(
    ...
    if frame_counter <= 3:
        logger.debug(
            f"[AI DEBUG] ball_hits_paddle_side=True, обрабатываем боковое столкновение"
        )
...
def handle_all_lives_lost_after_ball_loss(
    ...
    if training_mode:
        if not getattr(sys, "frozen", False):
            print(
                f"[GAME END] Все жизни потрачены (ball.rect.bottom > paddle.rect.top)! lives_left={lives_left}, training_mode={training_mode}"
            )
    new_game_over = True
...
def apply_restart_result(
    ...
    if new_paddle is not None:
        ...
        assert (
            new_ball is not None
        ), "new_ball must not be None when new_paddle is not None"
        assert (
            new_bricks is not None
        ), "new_bricks must not be None when new_paddle is not None"
        assert (
            new_score is not None
        ), "new_score must not be None when new_paddle is not None"
        assert (
            new_lives_left is not None
        ), "new_lives_left must not be None when new_paddle is not None"
        assert (
            new_game_over is not None
        ), "new_game_over must not be None when new_paddle is not None"
        assert (
            new_game_started is not None
        ), "new_game_started must not be None when new_paddle is not None"
        assert (
            new_game_start_time is not None
        ), "new_game_start_time must not be None when new_paddle is not None"
        return (
            new_paddle,
            new_ball,
            new_bricks,
...
def handle_victory_check(
    ...
    В авторежиме сразу перезапускает игру без показа экранов результатов.
...
def process_ball_physics_and_collisions(
    ...
    В авторежиме не показывает экраны результатов - сразу перезапускает игру.
    ...
    if ball_lost:
        ...
        if training_mode:
            logger.info(
                f"[LIFE LOSS] Жизни уменьшены! lives_left={lives_left}, game_over={game_over}"
            )
