...
if logger.level == logging.NOTSET:
    ...
    try:
        # Пытаемся импортировать как абсолютный импорт из src2.ai
        try:
...
class SettingsConstants:
    ...
    def validate_ball_speed_range(cls, speed: int) -> bool:
        ...
        """
        return cls.MIN_BALL_SPEED <= speed <= cls.MAX_BALL_SPEED
...
class SettingsManager:
    ...
    def set_ball_speed(self, speed: int) -> None:
        ...
        if not SettingsConstants.validate_ball_speed_range(speed):
            max_speed = SettingsConstants.get_max_ball_speed()
            raise ValueError(
