...
def render_game_frame(
    ...
    if frame_counter <= 3:
        ...
        logger.debug(
            f"[AI DEBUG] Начинаем отрисовку, game_over=False, bricks={len(bricks) if 'bricks' in locals() else 'N/A'}"
        )
    ...
    if frame_counter == 1:
        logger.debug(
            f"[AI DEBUG] Первый кадр отрисован, bricks={len(bricks)}, paddle.x={paddle.rect.x}, ball.x={ball.rect.centerx}, game_started={game_started}"
        )
