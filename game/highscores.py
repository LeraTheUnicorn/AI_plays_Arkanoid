...
if logger.level == logging.NOTSET:
    try:
        ...
        # Добавляем путь к родительскому каталогу (src2), чтобы можно было импортировать ai как модуль
        parent_dir = os.path.dirname(
            os.path.dirname(os.path.abspath(__file__))
        )  # src2/
        if parent_dir not in sys.path:
            sys.path.insert(0, parent_dir)
        from ai.logging_config import get_log_level  # type: ignore[import-untyped]
...
class HighScoreManager:
    ...
    def load_highscores(self) -> None:
        ...
        try:
            ...
            if not isinstance(data, list):
                logger.warning(
                    f"Файл {self.highscores_file} не содержит список, инициализируем пустой список"
                )
                self.highscores = []
                return
            ...
            logger.error(
                f"Ошибка декодирования JSON в файле {self.highscores_file}: {e}"
            )
            self.highscores = []
            ...
            logger.error(
                f"Неожиданная ошибка при загрузке рекордов: {e}", exc_info=True
            )
            self.highscores = []
        ...
        try:
            ...
            try:
                ...
                fallback_dir = os.path.join(
                    src_dir, "local_game_files"
                )  # src/local_game_files/
                ...
                logger.error(
                    f"Не удалось сохранить рекорды даже в fallback: {fallback_error}"
                )
    ...
    def add_score(self, player_name: str, score: int, game_time_seconds: int) -> bool:
        ...
        if not isinstance(score, int):
            raise TypeError(
                f"Очки должны быть целым числом, получен тип: {type(score).__name__}"
            )
        ...
        if not isinstance(game_time_seconds, int):
            raise TypeError(
                f"Время должно быть целым числом, получен тип: {type(game_time_seconds).__name__}"
            )
