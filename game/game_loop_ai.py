...
def update_ai_logic(
    ...
    if (
        ...
        and ai_player.separation_zone_tracker.game_restart_required
    ):
        # Нарушение правила - сбрасываем целевую позицию вместо перезапуска игры
        if not getattr(__import__("sys"), "frozen", False):
            print(
                f"[WARNING] Обнаружено нарушение правила фиксации позиции! Сбрасываем целевую позицию."
            )
    ...
    if training_mode:
        ...
        if frame_counter % FPS == 0:  # Каждую секунду (60 кадров)
            ...
            if current_ball_speed != optimal_ball_speed:
                ...
                if (
                    optimal_ball_speed > BALL_SPEED_MAX // 3
                ):  # Если больше трети максимума
                    ...
                    if ball.vel_x != 0:
                        ball.vel_x = int(
                            ball.vel_x * optimal_ball_speed / max(current_ball_speed, 1)
                        )
                    if ball.vel_y != 0:
                        ball.vel_y = int(
                            abs(ball.vel_y)
...
def update_ai_paddle_movement(
    ...
    target_position_set = (
        hasattr(ai_player, "separation_zone_tracker")
        and hasattr(ai_player.separation_zone_tracker, "target_position_set")
        and ai_player.separation_zone_tracker.target_position_set
    )
    ...
    if should_move and pygame.time.get_ticks() % 1000 < 16:  # Каждые ~1 секунду
        logger.debug(
            f"[PADDLE MOVEMENT DEBUG] ball_in_separation_zone={ball_in_separation_zone}, "
    ...
    if should_move:
        ...
        movement = ai_player.move_paddle_towards(paddle.rect.centerx, int(base_speed))

        # КРИТИЧНО: Логируем результат движения для диагностики (только в файл, не в консоль)
        if (
            movement == 0
            and target_position_set
            and pygame.time.get_ticks() % 1000 < 16
        ):
            target_pos = (
                ai_player.separation_zone_tracker.target_position
                if hasattr(ai_player, "separation_zone_tracker")
                else None
            )
            distance = (
                abs(paddle.rect.centerx - target_pos) if target_pos is not None else 0
            )
            logger.debug(
                f"[PADDLE MOVEMENT DEBUG] movement=0, но target_position_set=True! "
...
def apply_paddle_movement(
    ...
    if movement != 0:
        ...
        paddle.rect.centerx = max(min_center_x, min(max_center_x, new_center_x))
...
def process_paddle_control(
    ...
    if training_mode and ai_player is not None:
        ...
        if not training_mode:
            raise RuntimeError(
                "Игра работает только в режиме обучения (training_mode=True)"
            )
